...
/**
 *  An advanced, compact, lightweight, object-oriented MySQL database wrapper, built upon PHP's
 *  {@link http://www.php.net/manual/en/book.mysqli.php MySQLi extension}. It provides methods for interacting with MySQL
 *  databases that are more powerful and intuitive than PHP's default ones.
 *
 ...
 *  @version    2.8 (last revision: March 15, 2013)
...
class Zebra_Database
{
     ...
     *  TRUE the error messages are reported in the debug console);
     *
     *  @var string
     */
    var $log_path;
    ...
    function __construct()
    {
        ...
        $this->link_identifier = $this->memcache = $this->memcache_host = $this->memcache_port = false;
     ...
     *  @return void
     */
    function connect($host, $user, $password, $database, $connect = false)
    {
    ...
    function escape($string)
    {
        ...
        if ($this->_connected()) {
            ...
            return mysqli_real_escape_string($this->link_identifier, $string);
    ...
    function insert_id()
    {
        ...
        if ($this->_connected())
            ...
            if (isset($this->last_result)) return mysqli_insert_id($this->link_identifier);
    ...
    function query($sql, $replacements = '', $cache = false, $calc_rows = false, $highlight= false)
    {
        ...
        if ($this->_connected()) {
            ...
            if (!isset($this->last_result)) {
                ...
                // run the query
                $this->last_result = @mysqli_query($this->link_identifier, $sql);
                ...
                if ($this->_is_result($this->last_result) || $this->last_result === true) {
                    ...
                    if ($is_select) {
                        ...
                        if ($calc_rows) {
                            ...
                            $found_rows = mysqli_fetch_assoc(mysqli_query($this->link_identifier, 'SELECT FOUND_ROWS()'));
                    ...
                    } else $this->affected_rows = @mysqli_affected_rows($this->link_identifier);
                ...
                if ($this->debug) {
                    ...
                    if ($is_select) {
                        ...
                        if ($is_select && $this->_is_result($this->last_result)) {
                            ...
                            $explain_resource = mysqli_query($this->link_identifier, 'EXPLAIN EXTENDED ' . $sql);
    ...
    private function _connected()
    {
        ...
        if (!$this->link_identifier) {
            ...
            // select the database
            if (!(@mysqli_select_db($this->link_identifier, $this->credentials['database']))) {
